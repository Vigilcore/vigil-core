...
import { Shield, Brain, Lock, Eye, Zap, ChevronRight, CheckCircle2, AlertTriangle, Terminal, Info, BarChart3, Binary, Target, Activity, ShieldCheck, RefreshCcw, LogOut, Radar } from 'lucide-react';
...
export const NeuralProficiencyAudit: React.FC<{ onCompleteExit: () => void }> = ({ onCompleteExit }) => {
  ...
  const progressPercent = (Math.min(Object.keys(answers).length, REQUIRED_FOR_MESH) / REQUIRED_FOR_MESH) * 100;
  ...
  const isComplete = Object.keys(answers).length >= REQUIRED_FOR_MESH;
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-12">
          <div className="space-y-6 max-w-xl">
             <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                   ...
                   <h2 className="text-[2.25rem] md:text-[4.5rem] font-black text-white italic uppercase tracking-tighter leading-none">Neural Proficiency.</h2>
                </div>
             </div>
             <p className="text-zinc-400 text-lg font-medium italic leading-relaxed">
               "Verify your calibration against the VIGIL human-layer standard. A <span className="text-white">Rapid Audit (3 questions)</span> is sufficient to synchronize with the <span className="text-cyan-500">Mesh Intelligence Terminal</span>."
             </p>
          </div>
          ...
          <div className="w-full lg:w-80 p-8 bg-zinc-900/30 border border-zinc-800 rounded-[2.5rem] space-y-6 shadow-2xl">
             ...
             <div className="h-2 w-full bg-zinc-950 rounded-full overflow-hidden border border-zinc-900">
                <div className="h-full bg-cyan-500 shadow-[0_0_15px_cyan] transition-all duration-1000" style={{ width: `${progressPercent}%` }} />
             </div>
             <div className="flex justify-between items-center px-1">
                <p className="text-[9px] text-zinc-700 font-bold uppercase tracking-widest">Requirement: {Object.keys(answers).length}/{REQUIRED_FOR_MESH}</p>
                {isComplete && <div className="text-[9px] font-black text-emerald-500 uppercase italic animate-pulse">Mesh Link Secure</div>}
             </div>
          </div>
        </div>
                      ...
                      return (
                        <button 
                          ...
                          className={`aspect-square rounded-xl border flex items-center justify-center text-[11px] font-black transition-all duration-300 relative group ${
                            ...
                            isAnswered ? (isCorrect ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-500' : 'bg-red-600/10 border-red-600/40 text-red-500') : 
                 ...
                 {isComplete && (
                   <div className="absolute inset-0 z-50 bg-[#020202]/95 backdrop-blur-2xl flex flex-col items-center justify-center p-12 text-center space-y-12 animate-in zoom-in duration-700">
                      <div className="relative">
                         ...
                         <div className="absolute -top-4 -right-4 w-12 h-12 bg-emerald-500 text-black rounded-full flex items-center justify-center font-black italic">{Object.keys(answers).length}/25</div>
                      </div>
                      
                      <div className="space-y-4">
                         <h3 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter">Mesh Link <br/> Synchronized.</h3>
                         <p className="text-zinc-500 text-lg max-w-md mx-auto italic">
                           "Your neural clarity is now verified. Proceed to the <span className="text-cyan-500 font-bold">Mesh Terminal</span>—the system's most powerful tool for sovereign intelligence and technical assistance."
                         </p>
                      </div>
