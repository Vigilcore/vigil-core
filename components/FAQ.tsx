...
const FAQItem: React.FC<FAQItemProps> = ({ question, answer, category, index, icon, isOpen, onToggle }) => (
  <div className={`group border-b border-zinc-800 transition-all duration-500 ${isOpen ? 'bg-zinc-900/20' : 'hover:bg-zinc-900/10'}`}>
    <button 
      onClick={onToggle}
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <span className="text-[10px] font-mono font-black text-zinc-700 uppercase tracking-tighter">#{index}</span>
          <div className={`w-10 h-10 rounded-xl border border-zinc-700 flex items-center justify-center transition-all duration-500 ${isOpen ? 'bg-blue-600/10 border-blue-500/50 text-blue-500' : 'text-zinc-500 group-hover:text-zinc-300 group-hover:border-zinc-600'}`}>
        ...
        <div className="space-y-1">
          ...
          <h4 className={`text-sm md:text-lg font-black uppercase italic tracking-tight transition-colors ${isOpen ? 'text-white' : 'text-zinc-400 group-hover:text-zinc-200'}`}>
            {question}
          </h4>
        </div>
      </div>
  ...
  return (
    <section id="faq" className="px-6 md:px-20 py-12 bg-[#020202] relative z-10 scroll-mt-20">
      <div className="max-w-5xl mx-auto space-y-16">
        ...
        <div className="pt-12 flex flex-col md:flex-row items-center justify-between gap-8 border-t border-zinc-800">
           <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-3xl bg-zinc-900 flex items-center justify-center border border-zinc-700">
                 <MessageSquare className="w-6 h-6 text-zinc-500" />
              </div>
              <div className="space-y-1">
                 <p className="text-sm font-black text-white uppercase tracking-widest">Still have questions?</p>
                 <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Access the full documentation in our registry.</p>
              </div>
           </div>
           <button 
             onClick={() => onOpenDoc?.('technical_doc')}
             className="px-8 py-4 bg-zinc-900 border border-zinc-700 text-zinc-300 text-[10px] font-black uppercase tracking-widest rounded-3xl hover:text-white hover:border-blue-500/50 transition-all active:scale-95"
           >
             VIEW REGISTRY DOCS
           </button>
        </div>
